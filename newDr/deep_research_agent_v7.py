...
llm = ChatOpenAI(
    model=endpoint_id or "ep-placeholder",
    api_key=api_key,
    base_url="https://ark.cn-beijing.volces.com/api/v3",
    temperature=0.1,
) if api_key and endpoint_id else _DummyLLM()
...
async def retrieve_local_knowledge(state: ResearchState):
    """
    【节点 1.5 (V8 新增)：本地知识检索 (RAG 模拟)】
    根据当前查询，从本地知识库中检索相关信息。
    """
    if SIMULATED_LOCAL_KNOWLEDGE_BASE.strip() == "":
        print("⏭️ [本地检索] 知识库为空，跳过本地检索。")
        return {}

    queries = state["current_queries"]
    query_str = "\n".join(queries)

    print(f"\n🧠 [本地检索] 正在从模拟本地知识库中检索信息 (RAG)...")

    system_prompt = (
        "你是一个 RAG 检索器。根据给定的**查询**，从下方**本地知识文档**中提取所有相关的事实和数据。"
        "如果信息与查询无关，则忽略。"
        "提取的结果必须以 '### 来自本地知识库的 RAG 发现:' 开头，并使用格式【本地知识 X】进行引用。"
        "如果未找到相关信息，则回复 '无相关本地信息。'"
    )
    ...
    try:
        ...
        rag_finding = response.content.strip()

        if "无相关本地信息" in rag_finding:
            print("  ✅ 未检索到相关本地信息。")
            return {}
        else:
            print("  ✅ 成功检索到本地信息。")
            # 将 RAG 结果直接作为新的发现项添加到 all_findings
            new_findings = [rag_finding]
            total_findings = state.get("all_findings", []) + new_findings
            return {"all_findings": total_findings}

    except Exception as e:
        print(f"  ❌ 本地检索 (RAG) 失败: {e}")
        return {}
...
async def recursive_summarizer(state: ResearchState):
    ...
    system_prompt = (
        "你是一个递归记忆压缩引擎。你的任务是将在下方提供的旧研究发现（包含 web 发现和本地 RAG 结果），压缩成一个**单一、高层次、浓缩的摘要**。"
        "目标是保留核心事实和趋势，但移除不必要的细节，以便为新的研究发现腾出内存空间。"
        "压缩后的摘要必须以 '### 递归长期记忆摘要:' 开头。"
        "请务必在摘要中保留所有原始的【来源 X】或【本地知识 X】引用标记。"
        "只返回摘要内容，不带其他引导词。"
    )
...
async def evaluate_findings(state: ResearchState):
    """
    【节点 3：评估与反思】
    查看当前收集到的所有信息（包括本地 RAG 结果），判断是否足够写报告。
    """
    print("\n🤔 [评估] 正在检查资料完整性...")
    ...
    system_prompt = (
        "你是一个苛刻的研究导师。"
        "请阅读目前收集到的笔记（包括递归摘要、本地 RAG 结果和网络发现），判断是否足以撰写关于该主题的深度报告。"
...
async def outline_report(state: ResearchState):
    ...
    system_prompt = (
        ...
        "请根据这个类别和以下初步研究笔记（包含本地 RAG 结果），生成一份最专业、最相关的报告大纲。"
...
async def chapter_writer(state: ResearchState):
    ...
    system_prompt = (
        ...
        "写作时，必须使用上下文中的【来源 X】（网络）或【本地知识 X】（本地知识库）标记。"
...
def should_continue_research(state: ResearchState):
    ...
    if missing == "sufficient" or state["loop_count"] >= MAX_LOOPS:
        ...
        return "to_local_retriever"  # 循环回到本地检索
...
workflow.add_node("knowledge_retriever", retrieve_local_knowledge)  # [V8 新增] 本地知识检索节点
...
workflow.add_edge("planner", "knowledge_retriever")  # 1. 初始规划 -> 本地检索
workflow.add_edge("knowledge_retriever", "researcher")  # 2. 本地检索 -> 网络搜索
workflow.add_edge("researcher", "summarizer")  # 3. 网络搜索 -> 记忆压缩
workflow.add_edge("summarizer", "evaluator")  # 4. 记忆压缩 -> 评估
...
workflow.add_conditional_edges(
    ...
    {
        "to_local_retriever": "query_generator",  # 迭代：生成新查询
        "to_outline": "outline_planner"
    }
)

# 迭代循环：生成新查询 -> 本地检索 -> 网络搜索...
...
async def run_agent():
    print("=== Deep Research Agent V8 (本地知识 RAG + 递归记忆) ===")
    topic = input("请输入研究主题: ")
    ...
    # 保存文件
    with open("deep_research_v8.md", "w", encoding="utf-8") as f:
        f.write(final_state["final_report"])
    print("\n[系统] 报告已保存至 deep_research_v8.md")
